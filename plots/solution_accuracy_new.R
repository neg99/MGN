...
disperancy <- function(signal, v) {
    ...
    norm_signal <- signal / sqrt(sum(signal^2))

    sqrt(sum((norm_signal %*% compl_matrices(length(norm_signal), v, NULL)$s)^2))
    # sum(abs((norm_signal %*% compl_matrices(length(norm_signal), v, NULL)$s)))
}

bias <- function(signal, v) { #bias
    v <- v / sqrt(sum(v^2))
    signal <- as.numeric(signal)
    norm_signal <- signal / sqrt(sum(signal^2))

    abs(sum((norm_signal %*% compl_matrices(length(norm_signal), v, NULL)$s)))
}
...
eval_disperancy <- function(N, debug = FALSE) {
    ...
    list(mgn = distance(mgn_approx, signal),
         mgn_comp = distance(s_mgn_approx, signal),
         vp = distance(kostya_approx, signal),
         vp_comp = distance(kostya_approx_comp, signal),
         mgn_disp = disperancy(mgn_approx, v_mgn),
         mgn_comp_disp = disperancy(s_mgn_approx, v_s_mgn),
         vp_disp = disperancy(kostya_approx, v_vp),
         vp_comp_disp = disperancy(kostya_approx_comp, v_vp_comp),
...
compare_projectors <- function(N, debug = FALSE) {
    ...
    list(mgn = distance(mgn_proj, signal),

...
eval_for_one_N <- function(N, weights, vp_drop = Inf) {
    ...
    c(compare_time(series, v, it, signal, step_search = "mgn", weights = weights,
                   compensated = FALSE),
      compare_time(series, v, it, signal, step_search = "mgn", weights = weights),
        compare_time(series, v, it, signal, step_search = "vp",
...
legend("topleft", c("MGN", "MGN-H", "VPGN", "S-VPGN-H"),
...
legend("topright", c("MGN", "MGN-H", "VPGN", "S-VPGN-H"),
       lty = c(1,2,1,2), bg = rgb(1, 1, 1, 0.85), col = colors)
